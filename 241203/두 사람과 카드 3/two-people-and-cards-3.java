...
public class Main {
	public static void main(String[] args) throws Exception {
			...
			for (int j = 0; j <= N; j++) {
				for (int k = 0; k <= N; k++) {
					for (int m = 0; m <= M; m++) {
						...
						dp[next][next][k][m] = Math.min(dp[next][next][k][m], dp[i][j][k][m] + diff[j][next]);
						dp[next][j][next][m] = Math.min(dp[next][j][next][m], dp[i][j][k][m] + diff[k][next]);
		...
		for (int j = 0; j <= N; j++) {
			for (int k = 0; k <= N; k++) {
				answer = Math.min(answer, dp[N][j][k][M]);
			}
		}
